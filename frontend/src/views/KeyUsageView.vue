              ...
              <table class="w-full">
                ...
                <tbody>
                  ...
                  >
                    ...
                    <td class="px-4 py-3 text-sm tabular-nums text-right font-medium text-gray-900 dark:text-white">{{ formatMoney(row.actual_cost != null ? row.actual_cost : row.cost) }}</td>
                  </tr>
                </tbody>
              </table>
            </div>
              ...
              <table class="w-full">
                ...
                <tbody>
                  ...
                  >
                    ...
                    <td class="px-4 py-3 text-sm tabular-nums text-right font-medium text-gray-900 dark:text-white">{{ formatMoney(m.actual_cost != null ? m.actual_cost : m.cost) }}</td>
                  </tr>
                </tbody>
              </table>
            </div>
...
const ringItems = computed<RingItem[]>(() => {
  ...
  if (data.mode === 'quota_limited') {
    if (data.quota) {
      ...
      items.push({ title: t('keyUsage.totalQuota'), pct, amount: `${formatMoney(data.quota.used)} / ${formatMoney(data.quota.limit)}`, iconType: 'dollar' })
    }
    if (data.rate_limits) {
      ...
      for (const rl of data.rate_limits) {
        ...
        items.push({
          ...
          pct,
          amount: `${formatMoney(rl.used)} / ${formatMoney(rl.limit)}`,
    ...
    if (data.subscription) {
      ...
      for (const l of limits) {
        if (l.limit != null && l.limit > 0) {
          ...
          items.push({ title: l.label, pct, amount: `${formatMoney(l.usage)} / ${formatMoney(l.limit)}`, iconType: 'calendar' })
        }
      }
    }
  ...
  if (data.mode === 'quota_limited') {
    ...
    if (data.rate_limits) {
      ...
      for (const rl of data.rate_limits) {
        ...
        let valueStr = `${formatMoney(rl.used)} / ${formatMoney(rl.limit)}`
    ...
    if (data.subscription) {
      ...
      if (sub.daily_limit_usd > 0) {
        ...
        rows.push({
          iconBg: 'bg-primary-500/10', iconColor: 'text-primary-500', iconSvg: ICON_DOLLAR,
          label: `${t('keyUsage.usedQuota')} (${locale.value === 'zh' ? '日' : 'D'})`, value: `${formatMoney(sub.daily_usage_usd)} / ${formatMoney(sub.daily_limit_usd)}`, valueClass: getUsageColor(pct),
        })
      }
      if (sub.weekly_limit_usd > 0) {
        ...
        rows.push({
          ...
          label: `${t('keyUsage.usedQuota')} (${locale.value === 'zh' ? '周' : 'W'})`, value: `${formatMoney(sub.weekly_usage_usd)} / ${formatMoney(sub.weekly_limit_usd)}`, valueClass: getUsageColor(pct),
        })
      }
      if (sub.monthly_limit_usd > 0) {
        ...
        rows.push({
          ...
          label: `${t('keyUsage.usedQuota')} (${locale.value === 'zh' ? '月' : 'M'})`, value: `${formatMoney(sub.monthly_usage_usd)} / ${formatMoney(sub.monthly_limit_usd)}`, valueClass: getUsageColor(pct),
        })
      }
      if (sub.expires_at) {
        rows.push({
